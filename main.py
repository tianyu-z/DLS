...
nfs_dataset_path1 = "/mnt/nfs4-p1/ckx/datasets/"
nfs_dataset_path2 = "/nfs4-p1/ckx/datasets/"
nfs_dataset_path3 = "/mnt/nas/share2/home/lwh/cifar_datasets"

def main(
    dataset_path="/mnt/nas/share2/home/lwh/cifar_datasets",
    ...
    alpha=0.3,
    state_size=1,
    valuemodel_hiddensize=1,# 在replay buffer的初始化时会用到 state的维度
):
    sub_dict_keys = [
        "dataset_name",
        "image_size",
    ...
    sub_dict_str = "_".join([key + str(args[key]) for key in sub_dict_keys])
    wandb.login(key="831b4bf90cf69dcf8cae62953d13595412ce439d")
    run = wandb.init(project=project_name, name=sub_dict_str)
    wandb.config.update(args)
    run.save()
    ...
    for rank in range(args.size):
        ...
        if args.mode == "dqn_chooseone":
            value_model = load_valuemodel(
                args.state_size, args.valuemodel_hiddensize, args.size
            )  # 这里的144是pca weights的压缩后的维度 pca weights的shape是[144,144]

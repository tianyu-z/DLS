                        ...
                        for name, param in worker.model.named_parameters():
                            ...
                            if worker.rank == 0:
                                if name == "layer1.0.conv1.weight":
                                    work_weight_l0 = torch.zeros(
                                        (
                                            1,
                                            len(
                                                torch.nonzero(P_perturbed[worker.rank])
                                            ),
                                        )
                                    )
                                    for i in range(
                                        len(torch.nonzero(P_perturbed[worker.rank]))
                                    ):
                                        work_weight_l0[0, i] = torch.sum(
                                            param.grad
                                            * (
                                                dict(
                                                    worker_list[
                                                        torch.nonzero(
                                                            P_perturbed[worker.rank]
                                                        )[i]
